...
# AIO_WRITE(3) Linux Programmer's Manual
## NAME
...
The "asynchronous" means that this call returns as soon as the request has been enqueued; the write may or may not have completed when the call returns. One tests for completion using `aio_error(3)`. The return status of a completed I/O operation can be obtained by `aio_return(3)`. Asynchronous notification of I/O completion can be obtained by setting `aiocbp->aio_sigevent` appropriately; see `sigevent(7)` for details.
